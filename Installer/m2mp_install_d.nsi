...
Section "Install"
	...
	MessageBox MB_OK|MB_ICONSTOP "${MOD_DIR}"
	...
	File ..\Binary\debug\m2mp_d.exe
	File ..\Binary\debug\m2mp_d.dll
	...
	#File ..\Binary\guifiles\background.png
	...
	#SetOutPath "${MOD_DIR}"
	#CreateShortCut "$DESKTOP\${MOD_NAME}.lnk" "${MOD_DIR}\m2mp_d.exe"
	
	# Create the start menu shortcuts
	#CreateDirectory "$SMPROGRAMS\${MOD_NAME}"
	#CreateShortCut "$SMPROGRAMS\${MOD_NAME}\${MOD_NAME}.lnk" "${MOD_DIR}\m2mp_d.exe"
...
Function vcredist2010installer
	...
	vcredist_silent_install:
      ...
      vcredist_messagebox:
        MessageBox MB_OK "Failed to install Microsoft Visual C++ 2010 Redistributable ($INSTDIR\vcredist_x86.exe). Please ensure your system meets the minimum requirements before running the installer again."
        Goto vcredist_done
