...
void CPed::AddForPlayer ( EntityId playerId )
{
	...
	CCore::Instance()->GetNetworkModule()->Call( RPC_NEWPED, &bitStream, HIGH_PRIORITY, RELIABLE_ORDERED, playerId, false );
}
...
void CPed::RemoveForPlayer ( EntityId playerId )
{
	...
	// Send it to the player
	CCore::Instance()->GetNetworkModule()->Call( RPC_REMOVEPED, &bitStream, HIGH_PRIORITY, RELIABLE, playerId, false );
}
...
void CPed::SetModel ( int iModel, bool bSendToClient )
{
	...
	CCore::Instance()->GetNetworkModule()->Call( RPC_SETPEDMODEL, &bitStream, HIGH_PRIORITY, RELIABLE, INVALID_ENTITY_ID, true );
}