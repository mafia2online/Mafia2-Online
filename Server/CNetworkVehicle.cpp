...
CNetworkVehicle::CNetworkVehicle(void)
{
	...
	// Reset variables
	m_spawnProperties.m_uiModelIndex = -1;
	...
	m_spawnProperties.m_fDirtLevel = 0.0f;
	m_spawnProperties.m_iTuningTable = 0;
	m_spawnProperties.m_bWheelModels[0] = 0xFF;
	m_spawnProperties.m_bWheelModels[1] = 0xFF;
	m_spawnProperties.m_bWheelModels[2] = 0xFF;
	m_spawnProperties.m_fFuel = 100.0f;

	m_syncData.m_fEngineDamage = 0.0f;
	m_syncData.m_vecVelocity = CVector3();
	m_syncData.m_fTurnSpeed = 0.0f;
	m_syncData.m_Flags.bPower = false;
	m_syncData.m_Flags.bBrake = false;
	m_syncData.m_Flags.bPartState_Hood = 0;
	m_syncData.m_Flags.bPartState_Trunk = 0;
	m_syncData.m_Flags.bEngineState = false;
	m_syncData.m_Flags.bHornState = false;
	m_syncData.m_Flags.bSirenState = false;
	m_syncData.m_Flags.bLightState = false;

	memcpy(&m_spawnProperties.m_primaryColour, &predefinedColours[rand() % 20], sizeof(CColor));
	memcpy(&m_spawnProperties.m_secondaryColour, &predefinedColours[rand() % 20], sizeof(CColor));
	strcpy(m_spawnProperties.m_szPlateText, String("EB%d", m_vehicleId).Get());
}
...
void CNetworkVehicle::AddForPlayer(EntityId playerId)
{
	...
	pBitStream.Write(vecPos);
	pBitStream.Write(vecRot);
	pBitStream.Write((char *)&m_spawnProperties, sizeof(VehicleSpawnProperties));
...
void CNetworkVehicle::SetPosition(CVector3 vecPosition, bool bBroadcast)
{
	...
	// Store the position
	memcpy(&m_syncData.m_vecPosition, &vecPosition, sizeof(CVector3));
}

void CNetworkVehicle::GetPosition(CVector3 * vecPosition)
{
	// Copy the position
	memcpy(vecPosition, &m_syncData.m_vecPosition, sizeof(CVector3));
}
...
void CNetworkVehicle::SetRotation(CVector3 vecRotation, bool bBroadcast)
{
	...
	// Store the direction
	memcpy(&m_syncData.m_vecRotation, &vecRotation, sizeof(CVector3));
}

void CNetworkVehicle::GetRotation(CVector3 * vecRotation)
{
	// Copy the position
	memcpy(vecRotation, &m_syncData.m_vecRotation, sizeof(CVector3));
}
...
void CNetworkVehicle::SetColour(CColor primary, CColor secondary)
{
	...
	memcpy(&m_spawnProperties.m_primaryColour, &primary, sizeof(CColor));
	memcpy(&m_spawnProperties.m_secondaryColour, &secondary, sizeof(CColor));
}

void CNetworkVehicle::GetColour(CColor * primary, CColor * secondary)
{
	// Copy the primary colour
	memcpy(primary, &m_spawnProperties.m_primaryColour, sizeof(CColor));

	// Copy the secondary colour
	memcpy(secondary, &m_spawnProperties.m_secondaryColour, sizeof(CColor));
}
...
void CNetworkVehicle::SetPlateText(const char * szText)
{
	...
	strcpy(m_spawnProperties.m_szPlateText, szText);
}

const char * CNetworkVehicle::GetPlateText(void)
{
	return (const char *)m_spawnProperties.m_szPlateText;
}
...
void CNetworkVehicle::SetDirtLevel(float fDirtLevel)
{
	...
	m_spawnProperties.m_fDirtLevel = fDirtLevel;
}

float CNetworkVehicle::GetDirtLevel(void)
{
	return m_spawnProperties.m_fDirtLevel;
}
...
void CNetworkVehicle::SetEngineState(bool bState)
{
	...
	m_syncData.m_Flags.bEngineState = bState; // will be overwritten if synced
}

bool CNetworkVehicle::GetEngineState(void)
{
	return m_syncData.m_Flags.bEngineState;
}
...
void CNetworkVehicle::SetPartOpen(int iPart, bool bOpen)
{
	...
	case VEHICLE_PART_HOOD:
		m_syncData.m_Flags.bPartState_Hood = (int)bOpen;
		break;

	case VEHICLE_PART_TRUNK:
		m_syncData.m_Flags.bPartState_Trunk = (int)bOpen;
		break;
	}
}
...
bool CNetworkVehicle::IsPartOpen(int iPart)
{
	...
	{
	case VEHICLE_PART_HOOD:
		retn = m_syncData.m_Flags.bPartState_Hood;
		break;

	case VEHICLE_PART_TRUNK:
		retn = m_syncData.m_Flags.bPartState_Trunk;
		break;
	}
	return (retn);
}
...
void CNetworkVehicle::SetSirenState(bool bState)
{
	...
	m_syncData.m_Flags.bSirenState = bState;
}

bool CNetworkVehicle::GetSirenState(void)
{
	return m_syncData.m_Flags.bSirenState;
}
...
void CNetworkVehicle::SetBeaconLightState(bool bState)
{
	...
	// Store the last siren state
	m_syncData.m_Flags.bBeaconLightState = bState;
}

bool CNetworkVehicle::GetBeaconLightState(void)
{
	return (m_syncData.m_Flags.bBeaconLightState);
}
...
void CNetworkVehicle::SetHornState(bool bState)
{
	...
	m_syncData.m_Flags.bHornState = bState;
}

bool CNetworkVehicle::GetHornState(void)
{
	return m_syncData.m_Flags.bHornState;
}
...
void CNetworkVehicle::SetTuningTable(int iTable)
{
	...
	m_spawnProperties.m_iTuningTable = iTable;
}

int CNetworkVehicle::GetTuningTable(void)
{
	return m_spawnProperties.m_iTuningTable;
}
...
{
	return m_spawnProperties.m_bWheelModels[Math::Clamp< BYTE >(0, iWheelIndex, 3)];
}
...
void CNetworkVehicle::GetSpeedVec(CVector3 * vecSpeed)
{
	...
	memcpy(vecSpeed, &m_syncData.m_vecVelocity, sizeof(CVector3));
}
...
{
	return m_spawnProperties.m_fFuel;
}
...
{
	return m_syncData.m_Flags.bLightState;
}

void CNetworkVehicle::SetModel(int iModel)
{
	// Store the model id
	m_spawnProperties.m_uiModelIndex = iModel;
...
void CNetworkVehicle::HandlePlayerExit(CNetworkPlayer * pNetworkPlayer, int iSeat)
{
	...
	if (m_pOccupants[iSeat] && pNetworkPlayer->GetId() == m_pOccupants[iSeat]->GetId())
	{
		...
		if (iSeat == 0)
		{
			...
			m_syncData.m_Flags.bBrake = false;
			m_syncData.m_vecVelocity = CVector3();
			m_syncData.m_Flags.bHandbrake = true;
