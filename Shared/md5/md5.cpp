...
CMD5Hasher::CMD5Hasher( unsigned char * input, unsigned int length )
{
	Init();
	Update( input, length );
	Finalize( );
...
String md5( String str, bool bFile )
{
	CMD5Hasher md5;
	MD5 md5bytes;
	char result[33];
	...
	if( !bFile )
		md5.Calculate( str.Get(), str.GetLength(), md5bytes );
	else
		md5.Calculate( str.Get(), md5bytes );

	md5.ConvertToHex( md5bytes, result );
