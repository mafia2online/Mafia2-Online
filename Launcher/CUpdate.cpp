...
void CUpdate::FileWriteHandler ( unsigned int uiBytesWritten, void * pUserData )
{
	...
	// Update GUI
	pGUI->SetProgressValue ( (((float)pUpdater->m_uiTotalBytesWritten / (float)pUpdater->m_uiUpdateFileSize) * 100) );
	pGUI->SetWidgetText ( pGUI->GetWidget ( "total_label" ), String ( "%s / %s", SharedUtility::BytesToString ( pUpdater->m_uiTotalBytesWritten ).Get(), SharedUtility::BytesToString ( pUpdater->m_uiUpdateFileSize ).Get() ).Get() );
}
...
void CUpdate::CheckForUpdates ( void )
{
	...
	if ( m_bIsUpdateAvailable )
	{
		...
		{
			// Reset gui data
			pGUI->SetWidgetText ( pGUI->GetWidget ( "download_label" ), "Downloading update..." );
			pGUI->SetWidgetText ( pGUI->GetWidget ( "total_label" ), "" );
			pGUI->SetProgressValue ( 0 );
