...
void CCore::OnDeviceRender( void )
{
	...
	if( m_pClientScriptingManager && !m_pGUI->GetMainMenu()->IsVisible () )
	{
		...
		{
			if (i != pCore->GetPlayerManager()->GetLocalPlayer()->GetId() && pCore->GetPlayerManager()->IsActive(i) && pCore->GetPlayerManager()->Get(i)->GetPlayerPed()->IsOnScreen())
			{
				...
				if (fDistance <= 35.0){
					...
					String text = String("%s (%d)", pCore->GetPlayerManager()->Get(i)->GetNick(), i);
					...
					// We draw texts
					pCore->GetGraphics()->DrawTextA((vecScreen.fX - dimensionWidth / 2) + 1, vecScreen.fY + 1, (DWORD)0xFF000000, fScale, "tahoma-bold", false, text);
					pCore->GetGraphics()->DrawTextA((vecScreen.fX - dimensionWidth / 2), vecScreen.fY, pCore->GetPlayerManager()->Get(i)->GetColour(), fScale, "tahoma-bold", false, text);
