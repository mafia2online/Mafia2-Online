...
void CRemotePlayer::StoreOnFootSync( OnFootSync * onFootSync )
{
	...
	if( (vecLocalPos - onFootSync->m_vecPosition).Length() < 200.0f )
	{
		...
		/*if (GetAnimStyleDirectory() != onFootSync->m_styleDirectory || GetAnimStyleName() != onFootSync->m_styleName)
			SetAnimStyle(onFootSync->m_styleDirectory, onFootSync->m_styleName);
		*/
		// Update the handModel
		//if (GetHandModel() != onFootSync->m_iHandModel || GetHandModelHand() != onFootSync->m_iHand)R
